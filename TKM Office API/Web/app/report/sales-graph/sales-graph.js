angular.module('SmartShelve')
    .controller('SalesGraphCtrl',function($scope, $rootScope, toastr, SysMessageService, SearchQueryService, BreadcrumbService,
        ...
        $scope.fetchData = function (){
            var jsonResult = StockCardService.GetGraphSalesData($scope.param, function(){
                var data = jsonResult;
                $rootScope.safeApply(function () {
                    if (data.Total > 0) {
                        $scope.sales.Labels = data.Labels;
                        $scope.sales.Series = data.Series;
                        $scope.sales.Data = data.Data;
                        $scope.sales.Total = data.Total;
                    }
                    else {
                        $scope.sales = {
                            Labels: [],
